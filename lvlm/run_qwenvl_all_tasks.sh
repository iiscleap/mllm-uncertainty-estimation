...
# FESTA LVLM Experiments - QwenVL All Tasks Runner
# This script runs all QwenVL experiments for both BLINK and VSR datasets
...
function run_auc_calculation() {
    local dataset="$1"
    local csv_file="$2"
    local output_file="qwenvl_festa_auc_scores.txt"
    ...
    python kl_div_fusion.py --dataset "$dataset" --model qwenvl | tee -a "$output_file"
    echo "" | tee -a "$output_file"
...
echo "========================================="
echo "FESTA LVLM Experiments - QwenVL Runner"
echo "========================================="
echo "Start Time: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
echo "Starting QwenVL experiments..."
echo ""
...
run_task_with_progress "BLINK - Perturbed Questions (Perturb Sampling)" \
    ...
    python qwenvl_perturb_sampling.py \
...
echo "All QwenVL Vanilla experiments completed!"
echo ""
...
echo "==========================================" >> qwenvl_festa_auc_scores.txt
echo "Summary completed on: $(date '+%Y-%m-%d %H:%M:%S')" >> qwenvl_festa_auc_scores.txt
echo "All datasets processed: ${datasets[*]}" >> qwenvl_festa_auc_scores.txt
echo "==========================================" >> qwenvl_festa_auc_scores.txt

echo "========================================="
echo "FESTA LVLM Experiments - QwenVL Complete!"
echo "End Time: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
echo "Results saved in:"
echo "   - vanilla_output/"
echo "   - perturb_sampling_output/"
echo "   - qwenvl_festa_auc_scores.txt (AUC scores)"
echo "========================================="
