...
def plot_uncertainty_distribution(y_prob, y_true, combined_kl, model, dataset):
...
def compute_all_metrics_with_plots(model, dataset):
    """Compute all metrics and generate plots for a given model and dataset"""
    ...
    try:
        ...
        pos_kl, labels = calculate_kl_div(vanilla, pos_sample, [1.0, 0.0], answer_file)
        neg_kl, _ = calculate_kl_div(vanilla, neg_sample, [0.0, 1.0], answer_file)
        ...
        # Generate plots
        risk_cov_plot = plot_risk_coverage_desc(coverage, risk, model, dataset)
        acc_cov_plot = plot_accuracy_coverage(coverage, accuracy_curve, model, dataset)
        calib_plot = plot_calibration_curve(bin_confidences, bin_accuracies, bin_counts, model, dataset)
        dist_plot = plot_uncertainty_distribution(y_prob, y_true, combined_kl, model, dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, help="Name of the model (e.g., llava)")
    parser.add_argument("--dataset", type=str, choices=["blink", "vsr"], help="Dataset name")
    parser.add_argument("--all_models", action="store_true", help="Run for all available models")
    parser.add_argument("--all_datasets", action="store_true", help="Run for all datasets")
    ...
    if args.all_models:
        ...
        # Exclude files that start with "negated_" as these are negated sampling files, not separate models
        vanilla_files = os.listdir("vanilla_output/")
        models = set()
        for file in vanilla_files:
            if file.endswith(".txt") and "_vanilla_" in file and not file.startswith("negated_"):
                parts = file.split("_vanilla_")
                if len(parts) >= 2:
                    model = parts[0]
                    models.add(model)
    ...
    if args.all_datasets:
        ...
        if not args.dataset:
            parser.error("Either --dataset or --all_datasets must be specified")
    ...
    for model in models:
        for dataset in datasets:
            ...
            result = compute_all_metrics_with_plots(model, dataset)
            if result is not None:
                ...
                print(f"\nAbstention Analysis for {model}-{dataset}:")
    ...
    if results:
        ...
        # Print summary
        print(f"\nSUMMARY:")
        print(f"Total processed: {len(results)} model-dataset combinations")
        if len(results) > 1:
