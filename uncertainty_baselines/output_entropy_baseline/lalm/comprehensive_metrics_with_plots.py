...
def plot_risk_coverage_desc(coverage, risk, model, task, method, exp_type, save_dir):
    ...
    plt.title(f"Risk–Coverage for {model} on {task} [{method}|{exp_type}]")
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.xlim(1.0, 0.0)
    plt.ylim(0, 1)
    out = os.path.join(save_dir, f"{model}_{task}_{method}_{exp_type}_risk_coverage.png")
    plt.savefig(out, bbox_inches="tight")
    plt.close()
    return out

def plot_accuracy_coverage(coverage, accuracy, model, task, method, exp_type, save_dir):
    os.makedirs(save_dir, exist_ok=True)
    plt.figure(figsize=(7,5))
    plt.plot(coverage, accuracy, linewidth=2, color='g')
    plt.xlabel("Coverage (fraction answered)")
    plt.ylabel("Accuracy on Answered Set")
    plt.title(f"Accuracy vs. Coverage for {model} on {task} [{method}|{exp_type}]")
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.xlim(1.0, 0.0)
    plt.ylim(0, 1)
    out = os.path.join(save_dir, f"{model}_{task}_{method}_{exp_type}_accuracy_coverage.png")
    plt.savefig(out, bbox_inches="tight")
    plt.close()
    return out

def plot_calibration_curve(bin_confidences, bin_accuracies, bin_counts, model, task, method, exp_type, save_dir, num_bins=10):
    os.makedirs(save_dir, exist_ok=True)
    ...
    plt.ylabel('Accuracy')
    plt.title(f'Calibration Plot for {model} on {task} [{method}|{exp_type}]')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.tight_layout()
    plot_filename = os.path.join(save_dir, f"{model}_{task}_{method}_{exp_type}_calibration.png")
    plt.savefig(plot_filename, bbox_inches="tight", dpi=300)
    plt.close()
    return plot_filename

def plot_uncertainty_distribution(y_prob, y_true, model, task, method, exp_type, save_dir):
    os.makedirs(save_dir, exist_ok=True)
    ...
    plt.title(f'Confidence Score Distribution for {model} on {task} [{method}|{exp_type}]')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.5)
    out = os.path.join(save_dir, f"{model}_{task}_{method}_{exp_type}_uncertainty_distribution.png")
    plt.savefig(out, bbox_inches="tight", dpi=300)
    plt.close()
    return out

def main():
    parser = argparse.ArgumentParser(description='Comprehensive metrics and plots for LALM entropy baseline')
    parser.add_argument('--model', type=str, required=True)
    parser.add_argument('--task', type=str, required=True)
    parser.add_argument('--method', type=str, default='sampling')
    parser.add_argument('--exp_type', type=str, choices=['orig', 'neg'], default='orig')
    parser.add_argument('--data_dir', type=str, default='/home/debarpanb/VLM_project/malay_works/uncertainty_baselines')
    parser.add_argument('--output_dir', type=str, default='comprehensive_results')
    args = parser.parse_args()
    # File paths (identical to calculate_entropy_lalm_original.py)
    task = args.task
    model = args.model
    method = args.method
    exp_type = args.exp_type
    # Vanilla and uncertainty file logic:
    if exp_type == "orig":
    ...
    subset_file = f"subset/{task}_subset_100samples.csv"
    ...
    # Plots
    risk_cov_plot = plot_risk_coverage_desc(coverage, risk, args.model, args.task, args.method, args.exp_type, os.path.join(args.output_dir, 'risk_coverage_plots'))
    acc_cov_plot = plot_accuracy_coverage(coverage, accuracy_curve, args.model, args.task, args.method, args.exp_type, os.path.join(args.output_dir, 'risk_coverage_plots'))
    calib_plot = plot_calibration_curve(bin_confidences, bin_accuracies, bin_counts, args.model, args.task, args.method, args.exp_type, os.path.join(args.output_dir, 'ece_calibration_plots'))
    dist_plot = plot_uncertainty_distribution(y_prob, y_true, args.model, args.task, args.method, args.exp_type, os.path.join(args.output_dir, 'uncertainty_distribution_plots'))
    ...
    with open(summary_path, 'a', newline='') as f:
        ...
        if not file_exists:
            writer.writeheader()
        writer.writerow(row)
    print(f"Appended metrics for {args.model}-{args.task}-{args.method}-{args.exp_type} to {summary_path}")
