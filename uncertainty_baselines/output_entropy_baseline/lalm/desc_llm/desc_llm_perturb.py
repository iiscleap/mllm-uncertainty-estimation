...
def main():
    parser = argparse.ArgumentParser(description='Desc LLM perturb predictions for LALM')
    ...
    output_dir = f"{exp_type}/perturb"
    os.makedirs(output_dir, exist_ok=True)
    
    # Set output filename
    if exp_type == "orig":
        output_file = f"{output_dir}/desc_llm_{task}_perturb.txt"
    else:
        output_file = f"{output_dir}/negated_desc_llm_{task}_perturb.txt"
