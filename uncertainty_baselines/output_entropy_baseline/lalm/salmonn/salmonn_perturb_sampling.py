...
def main():
    parser = argparse.ArgumentParser(description='SALMONN perturb sampling predictions for LALM')
    ...
    # Set output filename
    if exp_type == "orig":
        output_file = f"{output_dir}/salmonn_{task}_perturb_sampling.txt"
    else:
        output_file = f"{output_dir}/negated_salmonn_{task}_perturb_sampling.txt"
            ...
            for row in reader:
                ...
                for i in range(k):
                    try:
                        ...
                        print(f"Error during generation for {idx} sample {i}: {e}")
                        response = "ERROR"
