...
def plot_risk_coverage_desc(coverage, risk, model, task, save_dir):
...
def plot_accuracy_coverage(coverage, accuracy, model, task, save_dir):
    os.makedirs(save_dir, exist_ok=True)
    plt.figure(figsize=(7,5))
...
def plot_calibration_curve(bin_confidences, bin_accuracies, bin_counts, model, task, save_dir, num_bins=10):
    os.makedirs(save_dir, exist_ok=True)
...
def plot_uncertainty_distribution(y_prob, y_true, model, task, save_dir):
    os.makedirs(save_dir, exist_ok=True)
...
def main():
    ...
    gt_path = f"/home/debarpanb/VLM_project/apoorva_works/Speech_LLM/temporal_dataset_augmented_100samples/{args.task}_task/{args.task}_subset_100samples.csv"
    pred_path = os.path.join(args.results_dir, f"{args.model}_guess_prob_{args.task}.csv")
    if not os.path.exists(pred_path):
        raise FileNotFoundError(f"Predictions file not found: {pred_path}")
    df_gt = pd.read_csv(gt_path)
    df_pred = pd.read_csv(pred_path)
    df = pd.merge(df_gt, df_pred, left_on='id', right_on='idx')
    y_true = (df['guess'] == df['correct']).astype(int).values
    ...
    # Plots
    risk_cov_plot = plot_risk_coverage_desc(coverage, risk, args.model, args.task, os.path.join(args.output_dir, 'risk_coverage_plots'))
    acc_cov_plot = plot_accuracy_coverage(coverage, accuracy_curve, args.model, args.task, os.path.join(args.output_dir, 'risk_coverage_plots'))
    calib_plot = plot_calibration_curve(bin_confidences, bin_accuracies, bin_counts, args.model, args.task, os.path.join(args.output_dir, 'ece_calibration_plots'))
    dist_plot = plot_uncertainty_distribution(y_prob, y_true, args.model, args.task, os.path.join(args.output_dir, 'uncertainty_distribution_plots'))
    ...
    with open(summary_path, 'a', newline='') as f:
        ...
        if not file_exists:
            writer.writeheader()
        writer.writerow(row)
    print(f"Appended metrics for {args.model}-{args.task} to {summary_path}")
