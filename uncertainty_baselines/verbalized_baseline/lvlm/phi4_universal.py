...
def main():
    parser = argparse.ArgumentParser(description='Run Phi4 verbalized confidence on spatial reasoning datasets')
    ...
    model_id = "microsoft/Phi-4-Vision"
    ...
    # Set output path
    output_path = os.path.join(args.output_dir, f"phi4_guess_prob_{args.dataset.lower()}.csv")
    ...
    with open(output_path, mode="w", newline="", encoding="utf-8") as out_file:
        ...
        for i in config['idx_range']:
            ...
            guess, prob = extract_guess_and_prob(decoded)
