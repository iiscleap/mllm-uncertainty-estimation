...
# Strictly reuse calculate_toggle.py logic for y_true, y_prob
def get_y_true_y_prob(model, dataset, exp_type):
    vanilla_exp = f"../../bahat_baseline_vlm/{model}_results/{model}_vanilla_{dataset}.txt"
    uncertainty_exp = f"../../bahat_baseline_vlm/{model}_results/{model}_{exp_type}_{dataset}.txt"
    if dataset == "vsr":
        answer_file = "../../bahat_baseline_vlm/vsr_data/answers_subset.txt"
    elif dataset == "blink":
        answer_file = "../../bahat_baseline_vlm/blink_data/answer_list.txt"
    else:
...
def plot_risk_coverage_desc(coverage, risk, model, dataset, exp_type, save_dir):
...
def main():
    ...
    # Plots
    risk_cov_plot = plot_risk_coverage_desc(coverage, risk, args.model, args.dataset, args.exp_type, os.path.join(args.output_dir, 'risk_coverage_plots'))
    acc_cov_plot = plot_accuracy_coverage(coverage, accuracy_curve, args.model, args.dataset, args.exp_type, os.path.join(args.output_dir, 'risk_coverage_plots'))
    calib_plot = plot_calibration_curve(bin_confidences, bin_accuracies, bin_counts, args.model, args.dataset, args.exp_type, os.path.join(args.output_dir, 'ece_calibration_plots'))
    dist_plot = plot_uncertainty_distribution(y_prob, y_true, args.model, args.dataset, args.exp_type, os.path.join(args.output_dir, 'uncertainty_distribution_plots'))
