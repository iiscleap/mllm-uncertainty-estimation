...
parser = argparse.ArgumentParser(description='Phi-4 top-k sampling baseline on BLINK')
parser.add_argument('--model_path', type=str, required=True, help='Path or HF ID for Phi-4-multimodal-instruct')
parser.add_argument('--qns_filepath', type=str, required=True, help='Path to BLINK questions CSV')
parser.add_argument('--img_folder', type=str, required=True, help='Path to BLINK orig_images folder')
parser.add_argument('--output_path', type=str, default='vlm_results/phi4_topk_sampling_blink.csv', help='Output CSV path')
