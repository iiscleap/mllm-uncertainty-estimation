...
def create_summary_tables(results_file, output_dir="summary_tables"):
    ...
    print(f"Loaded results for {len(df)} model-task combinations")
    ...
    # Create task-wise summary
    task_summary = []
    for task in df['Task'].unique():
        task_data = df[df['Task'] == task]
        task_row = {
            'Task': task,
            'Avg_Accuracy': task_data['Accuracy'].mean(),
            'Std_Accuracy': task_data['Accuracy'].std(),
            'Avg_AUROC': task_data['AUROC'].mean(),
            'Std_AUROC': task_data['AUROC'].std(),
            'Avg_AUPRC': task_data['AUPRC'].mean(),
            'Std_AUPRC': task_data['AUPRC'].std(),
            'Avg_Brier': task_data['Brier_Score'].mean(),
            'Std_Brier': task_data['Brier_Score'].std(),
            'Avg_ECE': task_data['ECE'].mean(),
            'Std_ECE': task_data['ECE'].std(),
            'Avg_AURC': task_data['AURC'].mean(),
            'Std_AURC': task_data['AURC'].std(),
        }
        task_summary.append(task_row)
    
    task_df = pd.DataFrame(task_summary)
    task_csv = os.path.join(output_dir, 'task_wise_summary.csv')
    task_df.to_csv(task_csv, index=False)
    print(f"Saved task-wise summary to {task_csv}")
    
    # Create visualizations
    create_summary_visualizations(df, summary_tables, output_dir)
    
    return summary_tables, overall_df, task_df
...
def create_summary_visualizations(df, summary_tables, output_dir):
    ...
    for i, metric in enumerate(metrics):
        if metric in summary_tables:
            ...
            axes[i].set_title(f'{metric.replace("_", " ")} by Model and Task')
    ...
    if 'AUROC' in summary_tables:
        ...
        plt.title('AUROC Heatmap: Models vs Tasks')
        plt.tight_layout()
    ...
    for task in df['Task'].unique():
        task_data = df[df['Task'] == task]
        plt.scatter(task_data['ECE'], task_data['AUROC'], 
                   label=f'Task: {task}', s=100, alpha=0.7)
    ...
    for i, metric in enumerate(metrics):
        if metric in df.columns:
            df.boxplot(column=metric, by='Task', ax=axes[i])
            axes[i].set_title(f'{metric.replace("_", " ")} Distribution by Task')
...
if __name__ == "__main__":
    ...
    summary_tables, overall_df, task_df = create_summary_tables(args.results_file, args.output_dir)
