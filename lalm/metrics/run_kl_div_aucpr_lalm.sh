...
OUTPUT_FILE="aucpr_results_lalm.txt"
> "$OUTPUT_FILE"

echo "Starting LALM AUCPR calculation run..."

# Loop through all model and task combinations
for model in "${MODELS[@]}"; do
    for task in "${TASKS[@]}"; do
        echo "Running AUCPR for $model on $task"
        
        python kl_div_aucpr.py --model "$model" --dataset "$task" >> "$OUTPUT_FILE"
...
echo "LALM AUCPR calculation finished. Results are in $OUTPUT_FILE"
