...
if __name__ == "__main__":
    ...
    parser.add_argument("--normalize_method", type=str, default="quantile", choices=["quantile", "minmax"], help="Normalization method")
    ...
    # Calculate accuracy
    accuracy = np.mean(labels1)
    print(f"Accuracy for {model}-{task}: {accuracy:.4f}")
    
    # Use specified normalization method
    if args.normalize_method == "quantile":
        y_prob = quantile_power_normalize(np.array(kl_only_list))
        print(f"Using quantile power normalization")
    else:
        y_prob = normalize_and_reciprocal(kl_only_list)
        print(f"Using min-max normalization")
    
    y_true = labels1

    auc_score = roc_auc_score(y_true, y_prob)
    print(f"AUROC for {model}-{task}: {auc_score:.4f}")
    
    print(f"First 5 y_prob values for {model}-{task}: {y_prob[:5]}")
