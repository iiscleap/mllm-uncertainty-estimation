...
# FESTA LALM Experiments - SALMONN All Tasks Runner
# This script runs all SALMONN experiments for count, duration, and order tasks
...
function run_auc_calculation() {
    local task="$1"
    local csv_file="$2"
    local output_file="salmonn_festa_auc_scores.txt"
    ...
    python kl_div_fusion.py --task "$task" --model salmonn | tee -a "$output_file"
    echo "" | tee -a "$output_file"
...
echo "========================================="
echo "FESTA LALM Experiments - SALMONN Runner"
echo "========================================="
echo "Start Time: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
# Activate salmonn conda environment
...
echo "FESTA AUC Scores - SALMONN Model" > salmonn_festa_auc_scores.txt
echo "Generated on: $(date '+%Y-%m-%d %H:%M:%S')" >> salmonn_festa_auc_scores.txt
echo "==========================================" >> salmonn_festa_auc_scores.txt
echo "" >> salmonn_festa_auc_scores.txt

echo "Starting SALMONN experiments..."
echo ""
...
for task in "${tasks[@]}"; do
    ...
    run_task_with_progress "$task - Original (Vanilla)" \
        ...
        python ./SALMONN/salmonn_vanilla.py \
        --task "$task" \
        --csv_path "dataset/TREA_dataset/$task/$task.csv" \
        --type "orig" \
        --wav_folder "dataset/TREA_dataset/$task/wav_folder"
    ...
    run_task_with_progress "$task - Original (Perturb Sampling)" \
        ...
        python ./SALMONN/salmonn_perturb_sampling.py \
        --task "$task" \
        --csv_path "dataset/TREA_dataset/$task/${task}_perturbed.csv" \
        --type "orig" \
        --wav_folder "dataset/TREA_dataset/$task/perturbed_wav_folder"
done
...
for task in "${tasks[@]}"; do
    ...
    run_task_with_progress "$task - Negated (Vanilla)" \
        ...
        --type "neg" \
        --wav_folder "dataset/TREA_dataset_negated/$task/wav_folder"
    ...
    run_task_with_progress "$task - Negated (Perturb Sampling)" \
        ...
        --type "neg" \
        --wav_folder "dataset/TREA_dataset_negated/$task/perturbed_wav_folder"
done

echo "All SALMONN experiments completed!"
echo ""
...
for task in "${tasks[@]}"; do
    run_auc_calculation "$task"
done

# Add summary to AUC scores file
echo "==========================================" >> salmonn_festa_auc_scores.txt
echo "Summary completed on: $(date '+%Y-%m-%d %H:%M:%S')" >> salmonn_festa_auc_scores.txt
echo "All tasks processed: ${tasks[*]}" >> salmonn_festa_auc_scores.txt
echo "==========================================" >> salmonn_festa_auc_scores.txt

echo "========================================="
echo "FESTA LALM Experiments - SALMONN Complete!"
echo "End Time: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
echo "Results saved in:"
echo "   - vanilla_output/"
echo "   - perturb_sampling_output/"
echo "   - salmonn_festa_auc_scores.txt (AUC scores)"
echo "========================================="
