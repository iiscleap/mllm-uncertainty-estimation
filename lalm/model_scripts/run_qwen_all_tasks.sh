...
# FESTA LALM Experiments - Qwen All Tasks Runner
# This script runs all Qwen experiments for count, duration, and order tasks
...
function run_auc_calculation() {
    local task="$1"
    local csv_file="$2"
    local output_file="qwen_festa_auc_scores.txt"
    ...
    python kl_div_fusion.py --task "$task" --model qwen | tee -a "$output_file"
    echo "" | tee -a "$output_file"
...
echo "========================================="
echo "FESTA LALM Experiments - Qwen Runner"
echo "========================================="
echo "Start Time: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
echo "FESTA AUC Scores - Qwen Model" > qwen_festa_auc_scores.txt
echo "Generated on: $(date '+%Y-%m-%d %H:%M:%S')" >> qwen_festa_auc_scores.txt
echo "==========================================" >> qwen_festa_auc_scores.txt
echo "" >> qwen_festa_auc_scores.txt

echo "Starting Qwen experiments..."
echo ""
...
for task in "${tasks[@]}"; do
    ...
    run_task_with_progress "$task - Original (Vanilla)" \
        ...
        --type "orig" \
        --wav_folder "dataset/TREA_dataset/$task/audios"
    ...
    run_task_with_progress "$task - Original (Perturb Sampling)" \
        ...
        --type "orig" \
        --wav_folder "dataset/TREA_dataset/$task/perturbed_audios"
done
...
for task in "${tasks[@]}"; do
    ...
    run_task_with_progress "$task - Negated (Vanilla)" \
        ...
        --type "neg" \
        --wav_folder "dataset/TREA_dataset_negated/$task/audios"
    ...
    run_task_with_progress "$task - Negated (Perturb Sampling)" \
        ...
        --type "neg" \
        --wav_folder "dataset/TREA_dataset_negated/$task/perturbed_audios"
done
...
echo "==========================================" >> qwen_festa_auc_scores.txt
echo "Summary completed on: $(date '+%Y-%m-%d %H:%M:%S')" >> qwen_festa_auc_scores.txt
echo "All tasks processed: ${tasks[*]}" >> qwen_festa_auc_scores.txt
echo "==========================================" >> qwen_festa_auc_scores.txt

echo "========================================="
echo "FESTA LALM Experiments - Qwen Complete!"
echo "End Time: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
echo "Results saved in:"
echo "   - vanilla_output/"
echo "   - perturb_sampling_output/"
echo "   - qwen_festa_auc_scores.txt (AUC scores)"
echo "========================================="
